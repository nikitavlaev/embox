...
static int usb_create_interface(struct usb_dev *dev, struct usb_desc_interface *iface_desc, void *end) {
...
static void usb_dev_fill_config(struct usb_dev *dev) {
	...
	while (cur < end) {
		...
		cur += usb_create_interface(dev, iface_desc, end);
